...
function Column({ columnId, columnTitle, scripts, script }) {
  ...
  const { setModalContent, setIsDarkMode } = useModal();
  ...
  const testScript = async (e) => {
    try {
      ...
      if (res.success) {
        const matches = res.payload.newMatches;

        if (matches.length > 0) {
          setModalContent(
            <>
              <h2>{matches.length} matches found for {testQuery}</h2>
              <ul>
                {matches.map((match, index) => (
                  <li key={index}>
                    {match.name} -&gt; price: {match.price}
                  </li>
                ))}
              </ul>
            </>
          ...
          setModalContent(
            <>
              <h2>No matches found for {testQuery}</h2>
            </>
          );
        }
      } else {
        setModalContent(
          <>
            <h2>Error: Unable to fetch matches</h2>
          </>
        );
      }
    } catch (error) {
