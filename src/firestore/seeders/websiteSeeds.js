...
const websiteData = [
  ...
  },
  {
    ...
    searchBarLocator: "input[placeholder='Search apparel, figures, and more']",
...
export const addWebsiteSeeds = async () => {
  const docRef = collection(db, "websites");

  websiteData.forEach((website) => {
    addDoc(docRef, website);
  });
};

export const deleteWebsiteSeeds = async () => {
  const querySnapshot = await getDocs(collection(db, "websites"));
  querySnapshot.forEach((website) => {
    deleteDoc(doc(db, "websites", website.id));
  });
};